...
import com.xmduruo.dto.Duihua;
import com.xmduruo.dto.Objectjson;
import com.xmduruo.dto.Task;
...
//        task.setTaskName(flowName);
    ...
    public static String getValueById(JsonObject jsonObject, Integer id) {
        ...
        if (null != jsonObject2.get("userInput")) {
            //得到userInput中的值去掉首尾的“”
            return com.xmduruo.util.StringUtils.trim(jsonObject2.get("userInput").getAsJsonArray().get(0).toString().trim(), '"');
        } else {
            return " ";
        }
    }
    ...
    public static String getUserQuery(JsonObject jsonObject, Integer id) {
        ...
        if (null != jsonArray3) {
            return com.xmduruo.util.StringUtils.trim(jsonArray3.get(jsonArray3.size() - 1).toString(), '"');
//            return jsonArray3.get(jsonArray3.size()-1).toString().replace("\"","");
        }
        return null;
    }

    public static String[] getMatchById(JsonObject jsonObject,Integer id){
        JsonObject jsonObject3 = WeChatParseJson.getJsonObjectById(id, jsonObject);
        JsonArray jsonArray3 = jsonObject3.getAsJsonArray("match");
        if(null !=jsonArray3){
            String[] match = com.xmduruo.util.StringUtils.trim(com.xmduruo.util.StringUtils.trim(jsonArray3.toString(), '['), ']').split(",");
    ...
    public static JsonObject parseWeChatJson(JsonObject json) {
        ...
        try {
            ...
            JsonObject json1 = json.getAsJsonObject("data");
    ...
    public static Map<String, String> electronicDigital(JsonObject jsonObject) {
        ...
        map.put("businessLicenseNumber", WeChatParseJson.getValueById(jsonObject, 8010));
        map.put("accountOfHousing", WeChatParseJson.getValueById(jsonObject, 8012));
        map.put("organizationCode", WeChatParseJson.getValueById(jsonObject, 8014));
        map.put("socialInsuranceNumber", WeChatParseJson.getValueById(jsonObject, 8016));
